...
public class BlockSculkDeepslate extends Block implements IGrowable {
	...
	public void updateTick(World worldIn, int x, int y, int z, Random rand) {
		if (!worldIn.isRemote) {
			if (worldIn.getBlockLightValue(x, y + 1, z) > 3 && worldIn.getBlockLightOpacity(x, y + 1, z) > 2) {
				worldIn.setBlock(x, y, z, OtherModBlocks.cobbled_deepslate);
	...
	public Item getItemDropped(int meta, Random random, int fortune) {
		return Item.getItemFromBlock(OtherModBlocks.cobbled_deepslate);
	}
	...
	public void registerBlockIcons(IIconRegister registry) {
		this.blockIcon = registry.registerIcon("nova_craft:sculk_deepslate");
	...
	public IIcon getIcon(int side, int meta) {
		return side == 1 ? this.blockIconTop : (side == 0 ? OtherModBlocks.deepslate.getBlockTextureFromSide(side) : this.blockIcon);
	}
