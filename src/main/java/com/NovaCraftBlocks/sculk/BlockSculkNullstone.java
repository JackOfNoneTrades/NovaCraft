...
public class BlockSculkNullstone extends Block implements IGrowable {

	@SideOnly(Side.CLIENT)
	private IIcon blockIconTop;

	public BlockSculkNullstone() {
		super(Material.grass);

		this.setHardness(3.45F);
		this.setResistance(3.45F);
	...
	public void registerBlockIcons(IIconRegister registry) {
		this.blockIcon = registry.registerIcon("nova_craft:sculk_nullstone");
		this.blockIconTop = registry.registerIcon("nova_craft:sculk_sensor_top");
	}

	@Override
	@SideOnly(Side.CLIENT)
	public IIcon getIcon(int side, int meta) {
		return side == 1 ? this.blockIconTop : (side == 0 ? NovaCraftBlocks.nullstone.getBlockTextureFromSide(side) : this.blockIcon);
	}
