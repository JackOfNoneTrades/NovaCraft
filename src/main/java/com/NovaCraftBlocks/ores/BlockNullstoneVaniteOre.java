...
public class BlockNullstoneVaniteOre extends Block {

	@SideOnly(Side.CLIENT)
	private IIcon iconFace, iconTop;

	public BlockNullstoneVaniteOre() {
		super(Material.rock);
		this.setHardness(6);
	...
	public void registerBlockIcons(IIconRegister iconRegister){
		blockIcon = iconRegister.registerIcon("nova_craft:nullstone_vanite_ore");
		iconFace = iconRegister.registerIcon("nova_craft:nullstone_vanite_ore");
		iconTop = iconRegister.registerIcon("nova_craft:nullstone_vanite_ore");
	}
	...
	public int quantityDroppedWithBonus(int fortune, Random random) {
		if (fortune > 0 && Item.getItemFromBlock(this) != this.getItemDropped(0, random, fortune)) {
			int j = random.nextInt(fortune + 4) - 1;
