...
public class BlockYttrlinsiteCluster extends BlockYttrlinsite implements ISubBlocksBlock {

	private final int type;
	@SideOnly(Side.CLIENT)
	private IIcon[] icons;

	public BlockYttrlinsiteCluster(int type) {
		super(Material.glass);
		setHardness(1.5F);
		setResistance(1.5F);
		this.setStepSound(ModSounds.soundCrystalCluster);
		setBlockName(Utils.getUnlocalisedName("yttrlinsite_cluster" + (type + 1)));
	...
	public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity entity) {
		if (entity instanceof EntityPlayer) {
			...
			((EntityLivingBase) entity).addPotionEffect(new PotionEffect(12, 500, 0));
    ...
    public int getLightValue(IBlockAccess world, int x, int y, int z)
    {
        ...
        return getLightValue() + (type * 4) + (meta / 6);
    }

   public Item getItemDropped(int p_149650_1_, Random p_149650_2_, int p_149650_3_)
   {
        return NovaCraftItems.yttrlinsite_shard;
    }
    ...
    {
    	if(this == NovaCraftBlocks.yttrlinsite_cluster_2 && meta >= 6) {
	...
	public void registerBlockIcons(IIconRegister p_149651_1_)
	{
		icons = new IIcon[2];
		if(type == 0) {
			icons[0] = p_149651_1_.registerIcon("nova_craft:yttrlinsite_bud");
			icons[1] = p_149651_1_.registerIcon("nova_craft:yttrlinsite_medium_bud");
		}
		if(type == 1) {
			icons[0] = p_149651_1_.registerIcon("nova_craft:yttrlinsite_large_bud");
			icons[1] = p_149651_1_.registerIcon("nova_craft:yttrlinsite_cluster");
		}
	...
	public int getRenderType() {
		return RenderIDs.YTTRLINSITE_CLUSTER;
	}

	@Override
	public Class<? extends ItemBlock> getItemBlockClass() {
		return ItemYttrlinsiteCluster.class;
	}
}
