...
public class BlockEchoCluster extends BlockEcho implements ISubBlocksBlock {

	private final int type;
	@SideOnly(Side.CLIENT)
	private IIcon[] icons;

	public BlockEchoCluster(int type) {
		super();
		setHardness(4F);
		setResistance(5F);
		this.setStepSound(ModSounds.soundCrystalCluster);
		setBlockName(Utils.getUnlocalisedName("echo_cluster" + (type + 1)));
	...
	public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity entity) {
		if (entity instanceof EntityPlayer) {
			...
			((EntityLivingBase) entity).addPotionEffect(new PotionEffect(15, 120, 0));
	...
	protected boolean canPlaceBlockOn(Block p_149854_1_)
	  {
	       return p_149854_1_ == (NovaCraftBlocks.budding_echo_block);
	  }
   ...
   {
        return NovaCraftItems.echo_shard;
    }
    ...
    {
    	if(this == NovaCraftBlocks.echo_cluster_2 && meta >= 6) {
	...
	public void registerBlockIcons(IIconRegister p_149651_1_)
	{
		icons = new IIcon[2];
		if(type == 0) {
			icons[0] = p_149651_1_.registerIcon("nova_craft:echo_bud");
			icons[1] = p_149651_1_.registerIcon("nova_craft:small_echo_bud");
		}
		if(type == 1) {
			icons[0] = p_149651_1_.registerIcon("nova_craft:medium_large_echo_bud");
			icons[1] = p_149651_1_.registerIcon("nova_craft:echo_cluster");
		}
	...
	public int getRenderType() {
		return RenderIDs.ECHO_CLUSTER;
	}

	@Override
	public Class<? extends ItemBlock> getItemBlockClass() {
		return ItemEchoCluster.class;
	}
}
