...
public class BlockBuddingLarimar extends BlockLarimar {

	public BlockBuddingLarimar() {
		setHardness(1.5F);
		setResistance(1.5F);
	...
	public void updateTick(World world, int x, int y, int z, Random rand) {
		...
		int meta = world.getBlockMetadata(x + facing.getFrontOffsetX(), y + facing.getFrontOffsetY(), z + facing.getFrontOffsetZ());
			...
			if(block instanceof BlockLarimarCluster && meta % 6 == facing.ordinal()) {
				...
				} else if(block == NovaCraftBlocks.larimar_cluster_1) {
					world.setBlock(x + facing.getFrontOffsetX(), y + facing.getFrontOffsetY(), z + facing.getFrontOffsetZ(), NovaCraftBlocks.larimar_cluster_2, meta - 6, 3);
				}
			} else if(canGrowIn(block)) {
				world.setBlock(x + facing.getFrontOffsetX(), y + facing.getFrontOffsetY(), z + facing.getFrontOffsetZ(), NovaCraftBlocks.larimar_cluster_1, facing.ordinal(), 3);
			}
		}
	}
