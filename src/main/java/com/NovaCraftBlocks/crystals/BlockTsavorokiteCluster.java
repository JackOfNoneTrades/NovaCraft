...
public class BlockTsavorokiteCluster extends BlockTsavorokite implements ISubBlocksBlock {

	private final int type;
	@SideOnly(Side.CLIENT)
	private IIcon[] icons;

	public BlockTsavorokiteCluster(int type) {
		super(Material.glass);
		setHardness(1.5F);
		setResistance(1.5F);
		this.setStepSound(ModSounds.soundCrystalCluster);
		setBlockName(Utils.getUnlocalisedName("tsavorokite_cluster" + (type + 1)));
	...
	public void onEntityCollidedWithBlock(World world, int x, int y, int z, Entity entity) {
		if (entity instanceof EntityPlayer) {
			...
			((EntityLivingBase) entity).addPotionEffect(new PotionEffect(8, 520, 0));
   ...
   {
        return NovaCraftItems.tsavorokite_shard;
    }
    ...
    {
    	if(this == NovaCraftBlocks.tsavorokite_cluster_2 && meta >= 6) {
	...
	public void registerBlockIcons(IIconRegister p_149651_1_)
	{
		icons = new IIcon[2];
		if(type == 0) {
			icons[0] = p_149651_1_.registerIcon("nova_craft:tsavorokite_bud");
			icons[1] = p_149651_1_.registerIcon("nova_craft:tsavorokite_medium_bud");
		}
		if(type == 1) {
			icons[0] = p_149651_1_.registerIcon("nova_craft:tsavorokite_large_bud");
			icons[1] = p_149651_1_.registerIcon("nova_craft:tsavorokite_cluster");
		}
	...
	public int getRenderType() {
		return RenderIDs.TSAVOROKITE_CLUSTER;
	}

	@Override
	public Class<? extends ItemBlock> getItemBlockClass() {
		return ItemTsavorokiteCluster.class;
	}
}
