...
public class BlockLarimarCluster extends BlockLarimar implements ISubBlocksBlock {

	private final int type;
	@SideOnly(Side.CLIENT)
	private IIcon[] icons;

	public BlockLarimarCluster(int type) {
		super(Material.glass);
		setHardness(1.5F);
		setResistance(1.5F);
		this.setStepSound(ModSounds.soundCrystalCluster);
		setBlockName(Utils.getUnlocalisedName("larimar_cluster" + (type + 1)));
   ...
   {
        return NovaCraftItems.larimar_shard;
    }
    ...
    {
    	if(this == NovaCraftBlocks.larimar_cluster_2 && meta >= 6) {
	...
	public void registerBlockIcons(IIconRegister p_149651_1_)
	{
		icons = new IIcon[2];
		if(type == 0) {
			icons[0] = p_149651_1_.registerIcon("nova_craft:larimar_bud");
			icons[1] = p_149651_1_.registerIcon("nova_craft:larimar_medium_bud");
		}
		if(type == 1) {
			icons[0] = p_149651_1_.registerIcon("nova_craft:larimar_large_bud");
			icons[1] = p_149651_1_.registerIcon("nova_craft:larimar_cluster");
		}
	...
	public int getRenderType() {
		return RenderIDs.LARIMAR_CLUSTER;
	}

	@Override
	public Class<? extends ItemBlock> getItemBlockClass() {
		return ItemLarimarCluster.class;
	}
}
