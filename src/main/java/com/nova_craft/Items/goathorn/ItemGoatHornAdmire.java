package com.nova_craft.Items.goathorn;
...
public class ItemGoatHornAdmire extends ItemBow {

	@SideOnly(Side.CLIENT)
	private IIcon[] iconArray;

	public ItemGoatHornAdmire() {
		super();
	...
	public void addInformation(final ItemStack stack, final EntityPlayer player, final List tooltip, final boolean who) {
        tooltip.add(EnumChatFormatting.GRAY + "" + StatCollector.translateToLocal("tooltip.horn.admire.desc"));
    }

	@Override
	public IIcon getIcon(ItemStack stack, int renderPass, EntityPlayer player, ItemStack usingItem, int useRemaining) {
		if (usingItem != null && stack.getItem() == NovaCraftItems.goat_horn_admire) {
	...
	public void onPlayerStoppedUsing(ItemStack stack, World worldIn, EntityPlayer entityLiving, int timeLeft) {
		if (entityLiving instanceof EntityPlayer) {
			...
			worldIn.playSound(entityplayer.posX, entityplayer.posY, entityplayer.posZ, "nova_craft:goat.horn.4", 5.0F, 0.8F + this.itemRand.nextFloat() * 0.3F, false);
			if (event.isCanceled()) {
				return;
			}
		}
