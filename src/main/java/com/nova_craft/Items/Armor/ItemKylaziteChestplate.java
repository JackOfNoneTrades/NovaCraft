...
public class ItemKylaziteChestplate extends ItemArmor implements ISpecialArmor
{
    public ItemKylaziteChestplate() {
        super(NCArmorMaterial.KYLAZITE, 0, 1);
        this.setCreativeTab(NovaCraftCreativeTabs.tools);
    }

    public String getArmorTexture(final ItemStack stack, final Entity entity, final int slot, final String type) {
        if (stack.getItem() == NovaCraftItems.kylazite_chestplate) {
            return "nova_craft:textures/armor/kylazite_layer_1.png";
        }
        FMLLog.log(Level.ERROR, "Invaild Texture for Kylazite Chestplate", new Object[0]);
        return null;
    }
	...
	public ArmorProperties getProperties(EntityLivingBase player, ItemStack armor, DamageSource source, double damage, int slot) {
    	ItemKylaziteChestplate armorItem = (ItemKylaziteChestplate) armor.getItem();
