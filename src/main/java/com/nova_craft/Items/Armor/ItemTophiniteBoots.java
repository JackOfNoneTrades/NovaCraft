...
public class ItemTophiniteBoots extends ItemArmor implements ISpecialArmor
{
    public ItemTophiniteBoots() {
        super(NCArmorMaterial.TOPHINITE, 0, 3);
        this.setCreativeTab(NovaCraftCreativeTabs.tools);
    }

    public String getArmorTexture(final ItemStack stack, final Entity entity, final int slot, final String type) {
        if (stack.getItem() == NovaCraftItems.tophinite_boots) {
            return "nova_craft:textures/armor/tophinite_layer_1.png";
        }
        FMLLog.log(Level.ERROR, "Invaild Texture for Tophinite Boots", new Object[0]);
        return null;
    }
    ...
    @Override
   	public ArmorProperties getProperties(EntityLivingBase player, ItemStack armor, DamageSource source, double damage, int slot) {
    	ItemTophiniteBoots armorItem = (ItemTophiniteBoots) armor.getItem();
