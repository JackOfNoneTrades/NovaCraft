...
public class ItemKylaziteLeggings extends ItemArmor implements ISpecialArmor
{
    public ItemKylaziteLeggings() {
        super(NCArmorMaterial.KYLAZITE, 0, 2);
        this.setCreativeTab(NovaCraftCreativeTabs.tools);
    }

    public String getArmorTexture(final ItemStack stack, final Entity entity, final int slot, final String type) {
        if (stack.getItem() == NovaCraftItems.kylazite_leggings) {
            return "nova_craft:textures/armor/kylazite_layer_2.png";
        }
        FMLLog.log(Level.ERROR, "Invaild Texture for Kylazite Leggings", new Object[0]);
        return null;
    }
	...
	public ArmorProperties getProperties(EntityLivingBase player, ItemStack armor, DamageSource source, double damage, int slot) {
    	ItemKylaziteLeggings armorItem = (ItemKylaziteLeggings) armor.getItem();
