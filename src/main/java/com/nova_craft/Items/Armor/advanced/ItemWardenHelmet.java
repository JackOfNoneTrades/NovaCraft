...
public class ItemWardenHelmet extends ItemArmor implements ISpecialArmor
{
	AttributeModifier health_increase_overworld = new AttributeModifier("generic.maxHealth", 2.0D, 0);

    public ItemWardenHelmet() {
        super(NCArmorMaterial.WARDEN, 0, 0);
        this.setCreativeTab(NovaCraftCreativeTabs.tools);
    }

    public String getArmorTexture(final ItemStack stack, final Entity entity, final int slot, final String type) {
        if (stack.getItem() == NovaCraftItems.warden_helmet) {
            return "nova_craft:textures/armor/warden_layer_1.png";
        }
        FMLLog.log(Level.ERROR, "Invaild Texture for Warden Helmet", new Object[0]);
        return null;
    }
	...
	public ArmorProperties getProperties(EntityLivingBase player, ItemStack armor, DamageSource source, double damage, int slot) {
    	ItemWardenHelmet armorItem = (ItemWardenHelmet) armor.getItem();
