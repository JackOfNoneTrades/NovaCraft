package com.nova_craft.Items.Armor.advanced;
...
public class ItemGuardianArmor extends ItemBaseGuardianArmor implements ISpecialArmor {
	...
	public ItemGuardianArmor(int armorType, ArmorMaterial material, String name, Item repair) {
		super(armorType, material, name, repair);
	}

	@Override
	public ArmorProperties getProperties(EntityLivingBase player, ItemStack armor, DamageSource source, double damage, int slot) {
		return new ArmorProperties(0, this.calculateIncrease(armor) / 22D, Integer.MAX_VALUE);
	}
	...
	private float calculateIncrease(ItemStack tool) {
		if (this.armorType == 0) {
			if (isBetween(tool, 0, 32)) {
				...
				return this.getDamageReductionAmount(3);
			} else if (isBetween(tool, 132, 21700)) {
				return this.getDamageReductionAmount(3);
			}
			...
			if (isBetween(tool, 0, 47)) {
				...
				return this.getDamageReductionAmount(4);
			} else if (isBetween(tool, 192, 21700)) {
				return this.getDamageReductionAmount(4);
			}
			...
			if (isBetween(tool, 0, 44)) {
				...
				return this.getDamageReductionAmount(3);
			} else if (isBetween(tool, 180, 21700)) {
				return this.getDamageReductionAmount(3);
			}
			...
			if (isBetween(tool, 0, 38)) {
				...
				return this.getDamageReductionAmount(3);
			} else if (isBetween(tool, 156, 21700)) {
				return this.getDamageReductionAmount(3);
			}
		}
	...
	public void onArmorTick(final World world, final EntityPlayer player, final ItemStack itemStack) {
        ...
        if (helmet != null) {
            hasCrystaliteHelmet = (helmet.getItem() == NovaCraftItems.guardian_helmet);
        }
        if (chest != null) {
            hasCrystaliteChest = (chest.getItem() == NovaCraftItems.guardian_chestplate);
        }
        if (legs != null) {
            hasCrystaliteLegs = (legs.getItem() == NovaCraftItems.guardian_leggings);
        }
        if (boots != null) {
            hasCrystaliteBoots = (boots.getItem() == NovaCraftItems.guardian_boots);
        }
