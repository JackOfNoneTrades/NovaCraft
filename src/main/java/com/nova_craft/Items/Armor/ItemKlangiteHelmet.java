...
public class ItemKlangiteHelmet extends ItemArmor implements ISpecialArmor
{
    public ItemKlangiteHelmet() {
        super(NCArmorMaterial.KLANGITE, 0, 0);
        this.setCreativeTab(NovaCraftCreativeTabs.tools);
    }

    public String getArmorTexture(final ItemStack stack, final Entity entity, final int slot, final String type) {
        if (stack.getItem() == NovaCraftItems.klangite_helmet) {
            return "nova_craft:textures/armor/klangite_layer_1.png";
        }
        FMLLog.log(Level.ERROR, "Invaild Texture for Klangite Helmet", new Object[0]);
        return null;
    }
	...
	public ArmorProperties getProperties(EntityLivingBase player, ItemStack armor, DamageSource source, double damage, int slot) {
    	ItemKlangiteHelmet armorItem = (ItemKlangiteHelmet) armor.getItem();
