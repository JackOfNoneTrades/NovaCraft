...
public class ItemCrystallizedVaniteBow extends ItemBow {

	@SideOnly(Side.CLIENT)
	private IIcon[] iconArray;

	public ItemCrystallizedVaniteBow() {
		super();
	...
	public IIcon getIcon(ItemStack stack, int renderPass, EntityPlayer player, ItemStack usingItem, int useRemaining) {
		if (usingItem != null && stack.getItem() == NovaCraftItems.crystallized_vanite_bow) {
	...
	public void registerIcons(IIconRegister p_94581_1_) {
		this.itemIcon = p_94581_1_.registerIcon(NovaCraft.find("crystallized_vanite_bow"));
			...
			if (itemstack != null || flag) {
				...
				if ((double) f >= 0.1D) {
					...
					if (!worldIn.isRemote) {
						...
						entityarrow.setDamage(entityarrow.getDamage() + (double) f * 1.25D + 1.25D);
