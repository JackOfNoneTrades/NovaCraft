...
public class ItemTophiniteBow extends ItemBow {

	@SideOnly(Side.CLIENT)
	private IIcon[] iconArray;

	public ItemTophiniteBow() {
		super();
	...
	public void addInformation(final ItemStack stack, final EntityPlayer player, final List tooltip, final boolean who) {
	 	   tooltip.add(EnumChatFormatting.DARK_AQUA + "" + StatCollector.translateToLocal("tooltip.tophinite_bow.desc"));
	}

	@Override
	public IIcon getIcon(ItemStack stack, int renderPass, EntityPlayer player, ItemStack usingItem, int useRemaining) {
		if (usingItem != null && stack.getItem() == NovaCraftItems.tophinite_bow) {
	...
	public void registerIcons(IIconRegister p_94581_1_) {
		this.itemIcon = p_94581_1_.registerIcon(NovaCraft.find("tophinite_bow"));
			...
			if (itemstack != null || flag) {
				...
				if ((double) f >= 0.1D) {
					...
					if (!worldIn.isRemote) {
						...
						entityarrow.setDamage(entityarrow.getDamage() + (double) f * 1.1D + 1.1D);
