...
public class ItemOphidianBow extends ItemBow {

	@SideOnly(Side.CLIENT)
	private IIcon[] iconArray;

	public ItemOphidianBow() {
		super();

		this.maxStackSize = 1;
		this.setMaxDamage(8688);
		this.setFull3D();
		this.setTextureName(NovaCraft.find("ophidian_bow"));
	...
	public IIcon getIcon(ItemStack stack, int renderPass, EntityPlayer player, ItemStack usingItem, int useRemaining) {
		if (usingItem != null && stack.getItem() == NovaCraftItems.ophidian_bow) {
	...
	public void registerIcons(IIconRegister p_94581_1_) {
		this.itemIcon = p_94581_1_.registerIcon(NovaCraft.find("ophidian_bow"));
	...
	public void addInformation(final ItemStack stack, final EntityPlayer player, final List tooltip, final boolean who) {
	 	   tooltip.add(EnumChatFormatting.LIGHT_PURPLE + "" + StatCollector.translateToLocal("tooltip.ophidian_bow.desc"));
	}
			...
			if (itemstack != null || flag) {
				...
				if ((double) f >= 0.1D) {
					...
					if (!worldIn.isRemote) {
						...
						entityarrow.setDamage(entityarrow.getDamage() + (double) f * 1.6D + 1.8D);
