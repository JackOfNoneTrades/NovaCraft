...
public class ItemCrystaliteBow extends ItemBow {

	@SideOnly(Side.CLIENT)
	private IIcon[] iconArray;

	public ItemCrystaliteBow() {
		super();

		this.maxStackSize = 1;
		this.setMaxDamage(6588);
		this.setFull3D();
		this.setTextureName(NovaCraft.find("crystalite_bow"));
	...
	public IIcon getIcon(ItemStack stack, int renderPass, EntityPlayer player, ItemStack usingItem, int useRemaining) {
		if (usingItem != null && stack.getItem() == NovaCraftItems.crystalite_bow) {
	...
	public void registerIcons(IIconRegister p_94581_1_) {
		this.itemIcon = p_94581_1_.registerIcon(NovaCraft.find("crystalite_bow"));
			...
			if (itemstack != null || flag) {
				...
				if ((double) f >= 0.1D) {
					...
					if (!worldIn.isRemote) {
						...
						entityarrow.setDamage(entityarrow.getDamage() + (double) f * 2.2D + 2.2D);
