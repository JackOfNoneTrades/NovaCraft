...
public class ItemKlangiteFirechargeLauncher extends ItemBow {

	@SideOnly(Side.CLIENT)
	private IIcon[] iconArray;

	public ItemKlangiteFirechargeLauncher() {
		super();

		this.maxStackSize = 1;
		this.setMaxDamage(2237);
		this.setFull3D();
		this.setTextureName(NovaCraft.find("klangite_firechargelauncher"));
	...
	public void addInformation(final ItemStack stack, final EntityPlayer player, final List tooltip, final boolean who) {
        tooltip.add(EnumChatFormatting.LIGHT_PURPLE + "" + StatCollector.translateToLocal("tooltip.klangite_firecharge.desc"));
    }

	@Override
	public IIcon getIcon(ItemStack stack, int renderPass, EntityPlayer player, ItemStack usingItem, int useRemaining) {
		if (usingItem != null && stack.getItem() == NovaCraftItems.klangite_firecharge_launcher) {
	...
	public void registerIcons(IIconRegister p_94581_1_) {
		this.itemIcon = p_94581_1_.registerIcon(NovaCraft.find("klangite_firechargelauncher"));
	...
	protected boolean isArrow(ItemStack stack) {
		return stack != null && stack.getItem() == NovaCraftItems.xancium_firecharge;
	}
	...
	public void onPlayerStoppedUsing(ItemStack stack, World worldIn, EntityPlayer entityLiving, int timeLeft) {
		if (entityLiving instanceof EntityPlayer) {
			...
			if (itemstack != null || flag) {
				if (itemstack == null) {
					itemstack = new ItemStack(NovaCraftItems.xancium_firecharge);
				}
				...
				if ((double) f >= 0.1D) {
					...
					if (!worldIn.isRemote) {
						float f2 = 2.0F;
			        	final EntityKlangiteFirechargeProjectile diamond_firecharge = new EntityKlangiteFirechargeProjectile(worldIn, (EntityLivingBase)entityLiving, 1.3f);
	...
	public EntityKlangiteFirechargeProjectile createArrow(World worldIn, float distance, ItemStack stack, EntityLivingBase shooter) {
		EntityKlangiteFirechargeProjectile entityDiamondArrow = new EntityKlangiteFirechargeProjectile(worldIn, shooter, distance);
