...
public class LivingEnderIsland extends WorldGenerator {
   int bumpsWide1;
   int bumpsLong1;
   ...
   private Block materialAtLevel(Random rand, int i, boolean flag) {
      ...
      if (i == j) {
          ...
          if (rand3 == 1 ) {
        	  return rand.nextInt(14) == 0 ? Blocks.end_stone : Blocks.end_stone;
          }
        	  return rand.nextInt(10) == 0 ? NovaCraftBlocks.frontierslate  : Blocks.end_stone;
        }
      else {
         return (Block)(flag ? NovaCraftBlocks.living_ender : Blocks.end_stone);
      }
   }
      ...
      for(int i = 0; i < 8 + rand.nextInt(radius); ++i) {
         ...
         if (world.getBlock(x1, y1, z1).isReplaceableOreGen(world, x, y, z, Blocks.end_stone)) {
            ...
            for(int yTest = y1; yTest < 128; ++yTest) { //128
               if (world.getBlock(x1, yTest, z1).isReplaceable(world, x1, yTest, z1) && (world.getBlock(x1, yTest - 1, z1) == Blocks.end_stone || world.getBlock(x1, yTest - 1, z1) == Blocks.end_stone)) {
                  world.setBlock(x1, yTest, z1, NovaCraftBlocks.ender_sprout);
                  break;
               }
            }
         }
   ...
   private static LivingEnderEndgusGen ender = new LivingEnderEndgusGen(Blocks.air, 1, true);
   ...
   private void generateTrees(World world, Random rand, int x, int y, int z) {
	   int radius = this.bumpsLong1 > this.bumpsWide1 ? this.bumpsLong1 : this.bumpsWide1;
	      ...
	      int k1;
	      int l1;
	      for(i6 = 0; i6 < 1 + rand.nextInt(radius); ++i6) {
	         ...
	         if (l1 > 15) {
	            if (rand.nextInt(250) == 5) {
	            	ender.generate(world, rand, j1, l1, k1);
	            }
	         }
   ...
   private void generateTallGrass(World world, Random rand, int x, int y, int z) {
      for(int i1 = 0; i1 < 16; ++i1) {
        ...
        break;
        case 2: (new WorldGenTallGrass(NovaCraftBlocks.ender_fungus, 1)).generate(world, rand, j1, l1, k1);
        break;
        case 3: (new WorldGenTallGrass(NovaCraftBlocks.ender_brain, 1)).generate(world, rand, j1, l1, k1);
        break;
        default:
        	break;
        }
