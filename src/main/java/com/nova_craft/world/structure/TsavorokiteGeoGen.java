...
public class TsavorokiteGeoGen extends WorldGenerator {
	//Credit Goes to Roadhog360

	@Override
	public boolean generate(World world, Random random, int x, int y, int z) {
		final float size = (random.nextInt(1) + 4) + random.nextFloat();
            ...
            for (int j = -sizeInt; j <= sizeInt; j++) {
                for (int k = -sizeInt; k <= sizeInt; k++) {
                    ...
                    if (distSq <= DISTANCE_INNER_SQ) {
                        ...
                        world.setBlock(x + i, y + j, z + k, NovaCraftBlocks.cobbled_grimstone, 0, 2);
	...
	private void placeAmethyst(World world, Random random, int x, int y, int z) {
		if(random.nextInt(12) == 0) {
			world.setBlock(x, y, z, NovaCraftBlocks.budding_tsavorokite_block);
			for(EnumFacing facing : EnumFacing.values()) {
				...
				if(clusterSize > 0) {
					...
					if(world.getBlock(offX, offY, offZ).getMaterial() == Material.air || world.getBlock(offX, offY, offZ).getMaterial() == Material.water) {
						Block block = clusterSize > 2 ? NovaCraftBlocks.tsavorokite_cluster_1 : NovaCraftBlocks.tsavorokite_cluster_2;
