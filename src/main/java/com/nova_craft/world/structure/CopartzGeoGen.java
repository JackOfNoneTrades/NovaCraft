...
public class CopartzGeoGen extends WorldGenerator {
	...
	private void placeAmethyst(World world, Random random, int x, int y, int z) {
		if(random.nextInt(12) == 0) {
			world.setBlock(x, y, z, NovaCraftBlocks.budding_copartz_block);
			for(EnumFacing facing : EnumFacing.values()) {
				...
				if(clusterSize > 0) {
					...
					if(world.getBlock(offX, offY, offZ).getMaterial() == Material.air || world.getBlock(offX, offY, offZ).getMaterial() == Material.water) {
						Block block = clusterSize > 2 ? NovaCraftBlocks.copartz_cluster_1 : NovaCraftBlocks.copartz_cluster_2;
			...
			world.setBlock(x, y, z, NovaCraftBlocks.copartz_block);
		}
	}
