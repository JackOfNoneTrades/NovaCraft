package com.nova_craft.world.village;
...
public class VindicatorHouseSmallNoDoor extends WorldGenerator
{
	...
	public boolean generate(World world, Random rand, int i, int j, int k) {
		...
		if(rand.nextInt(2) == 0){
			...
			TileEntityChest chest = (TileEntityChest) world.getTileEntity(i + 3, j + 2, k + 3);

			for (int slot = 0; slot < 3 + rand.nextInt(25); slot++) {
				chest.setInventorySlotContents(rand.nextInt(chest.getSizeInventory()), this.getFoodLoot(rand));
			}
		}
