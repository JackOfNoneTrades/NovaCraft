...
import com.nova_craft.Hardmode;
...
public class EntityEnderLord extends EntityMob
{
    ...
    public void onEntityUpdate() {
    	...
    	int k;
    	if (Configs.enableEnderLordParticles) {
        	for (k = 0; k < 2; ++k)
        	{
        		ParticleHandler.ENDERLORD.spawn(worldObj, this.posX + (this.rand.nextDouble() - 0.5D) * (double)this.width, this.posY + this.rand.nextDouble() * (double)this.height - 0.25D, this.posZ + (this.rand.nextDouble() - 0.5D) * (double)this.width);
        	}
